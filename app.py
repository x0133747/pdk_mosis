...
DESIGN_RULES = {
    ...
    "METAL1": {
        ...
        "7.4": {
            "desc": "Minimum spacing when wide (>10λ)",
            "value": 6,
            "type": "spacing",
        },
    },
...
def qa_generator_app():
    st.markdown("### 🔍 Pass/Fail Error Cells GDS QA Generator")
    st.markdown(
        ...
        for category, rules in DESIGN_RULES.items():
            with st.expander(f"{category} Rules"):
                for rule_id, rule_info in rules.items():
                    if st.checkbox(
                        f"{rule_id}: {rule_info['desc']} ({rule_info['value']}λ)",
    ...
    with col2:
        ...
        if st.session_state.generated_gds:
                    ...
                    summary_data.append(
                        {
                            ...
                            "Nominal": f"{info['value']}λ",
                            "Violated": f"{info['value'] - 0.5}λ",
                        }
                    )
            ...
            with col_klayout:
                if st.button(
                    "🔍 Open in KLayout", key="btn_klayout", use_container_width=True
                ):
            ...
            st.info(
                "💡 All cells contain intentional DRC violations and should FAIL DRC checks."
            )
        else:
...
def generate_calibre_deck(lambda_val=1.0, categories=None):
    ...
    lines = []
    lines.append("////////////////////////////////////////////////////////")
    lines.append("// MOSIS SCMOS SUBM Calibre DRC Rules Deck")
    lines.append(f"// Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    lines.append(f"// Lambda: {lambda_val} μm")
...
def calibre_generator_app():
    st.markdown("### 📜 Siemens Calibre DRC Rules Deck Generator")
    ...
    with col2:
        st.subheader("Generated Rules Deck")
...
ACT_SPACE = ACTIVE SEPARATION < 1.5 SERIOUS
    ...
    st.markdown("### 📖 Calibre SVRF User Reference Manual")
    st.markdown(
    ...
    with col4:
        ...
        if st.button("📄 View Calibre SVRF", type="primary", use_container_width=True):
    ...
    st.markdown("### 📋 Standard Basis Rules Summary")

    with st.expander("📐 Basic SVRF Rule Types", expanded=True):
        st.markdown("""
        #### WIDTH Rules
        - **Purpose**: Measure the minimum width of a layer
        - **Syntax**: `INTERNAL layer < value [ABUT < angle] [SINGULAR]`
        - **Example**: `INTERNAL POLY < 2.0` - checks poly minimum width is 2.0 μm
        ...
        - **Example**: `EXTERNAL METAL1 < 3.0` - checks metal1 minimum spacing is 3.0 μm
        ...
        - **Example**: `ENCLOSURE METAL1 VIA < 1.0` - checks metal1 overlaps via by 1.0 μm
        - **Common Use**: Contact overlap, via enclosure, implant overlap checks
        """)

    with st.expander("⚙️ SVRF Syntax Quick Reference"):
        st.markdown("""
    ...
    st.markdown("### 📖 Calibre Verification User's Manual")
    ...
    with col6:
        st.markdown("#### View Document")
        if st.button(
            "📄 View Calibre Verification User's Manual",
            type="primary",
            use_container_width=True,
        ):
...
def calibre_lvs_generator_app():
    """Calibre LVS Rules Deck Generator - Generate LVS rule decks for SCMOS SUBM"""
    st.markdown("### 🔍 Siemens Calibre LVS Rules Deck Generator")
    ...
    with col1:
        ...
        st.markdown("#### Rules Deck Options")
        ...
        st.markdown("---")
        if st.button("Generate LVS Rules Deck", type="primary"):
    ...
    with col2:
        st.subheader("Generated LVS Rules Deck")
        ...
        if st.session_state.lvs_deck:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            st.download_button(
                label="📥 Download Calibre LVS Rules Deck (.rules)",
            ...
            st.info("Click 'Generate LVS Rules Deck' to load the rule deck")
...
PROPERTY MN(NMOS) {
    ...
    if st.session_state.lvs_test_gds:
        st.markdown("---")
        st.markdown("### 📐 Generated Test Layout Structures (GDS)")
        ...
        with col_klayout:
            if st.button(
                "🔍 Open in KLayout", key="btn_lvs_klayout", use_container_width=True
            ):
                success, msg = launch_gds_viewer(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        st.download_button(
            label="💾 Download LVS Test Layouts (GDS)",
        ...
        st.info(
            "💡 Use these test structures to validate LVS rule deck extraction and device recognition."
        )

        st.markdown("---")

        # Sample SPICE Netlist Format
        st.markdown("### 📝 Sample SPICE Netlist Format")
...
"""
    ...
    # LVS Basics Summary
    st.markdown("### 📋 LVS Basic Operations Summary")

    with st.expander("🔧 Device Recognition", expanded=True):
        st.markdown(
            """
        #### DEVICE Statement
    ...
    with st.expander("⚡ Property Extraction"):
        st.markdown(
            """
        #### PROPERTY Statement
    ...
    with st.expander("🔗 Connectivity Extraction"):
        st.markdown(
            """
        #### CONNECT Statement
    ...
    with st.expander("⚙️ LVS Comparison Settings"):
        st.markdown(
            """
    ...
    st.markdown("### 📖 Calibre Verification User Manual")
    ...
    with col4:
        st.markdown("#### View Document")
        if st.button(
            "📄 View Calibre Verification", type="primary", use_container_width=True
        ):
...
# ============== APP 4: CALIBRE PEX/xCALIBRE DECK GENERATOR ==============


def calibre_xcalibre_generator_app():
    """Calibre PEX/xCalibrate Rules Deck Generator - Generate xCalibrate batch rule decks for MOSIS SCMOS SUBM"""
    st.markdown("### 🔬 Siemens Calibre PEX/xCalibrate Rules Deck Generator")
    st.markdown(
        "Generate xCalibrate batch format parasitic extraction rule decks for MOSIS SCMOS SUBM technology"
    )

    # Initialize session state for xCalibrate decks
    if "xcalibre_cap_file" not in st.session_state:
        st.session_state.xcalibre_cap_file = None
    if "xcalibre_res_file" not in st.session_state:
        st.session_state.xcalibre_res_file = None
    if "xcalibre_rcx_file" not in st.session_state:
        st.session_state.xcalibre_rcx_file = None
    ...
    with col1:
        ...
        st.markdown("#### MIPT Options")
        mipt_version = st.selectbox(
            "MIPT Format Version",
            ["MIPT 2.0", "MIPT 1.0"],
            index=0,
            help="Select Multi-Interval Parasitic Table format version",
        )
        ...
        st.markdown("---")
        if st.button("Generate xCalibrate Rules Decks", type="primary"):
            with st.spinner("Generating xCalibrate rule decks..."):
                try:
                    # Generate three files
                    cap_content, res_content, rcx_content = generate_xcalibre_decks(
                        extraction_type,
                        include_resistance,
                        include_capacitance,
                        include_coupling,
                    ...
                    st.session_state.xcalibre_cap_file = cap_content
                    st.session_state.xcalibre_res_file = res_content
                    st.session_state.xcalibre_rcx_file = rcx_content
                    st.success("xCalibrate rule decks generated successfully!")
                except Exception as e:
                    st.error(f"Error generating xCalibrate decks: {str(e)}")

    with col2:
        st.subheader("Generated xCalibrate Rules Decks")

        cap_file = st.session_state.xcalibre_cap_file
        res_file = st.session_state.xcalibre_res_file
        rcx_file = st.session_state.xcalibre_rcx_file

        if cap_file and res_file and rcx_file:
            # Create tabs for the three files
            tab1, tab2, tab3 = st.tabs(
                ["SCMOS_rules.C", "SCMOS_rules.R", "SCMOS_rcx.cal"]
            )

            with tab1:
                st.code(cap_file, language="text")
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                st.download_button(
                    label="📥 Download SCMOS_rules.C",
                    data=cap_file,
                    file_name=f"SCMOS_rules_{timestamp}.C",
                    mime="text/plain",
                    key="download_cap",
                )

            with tab2:
                st.code(res_file, language="text")
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                st.download_button(
                    label="📥 Download SCMOS_rules.R",
                    data=res_file,
                    file_name=f"SCMOS_rules_{timestamp}.R",
                    mime="text/plain",
                    key="download_res",
                )

            with tab3:
                st.code(rcx_file, language="text")
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                st.download_button(
                    label="📥 Download SCMOS_rcx.cal",
                    data=rcx_file,
                    file_name=f"SCMOS_rcx_{timestamp}.cal",
                    mime="text/plain",
                    key="download_rcx",
                )
            ...
            st.markdown(
                """
            - [ ] Verify MIPT 2.0 table format is correct
            - [ ] Check layer mapping matches PDK
            - [ ] Validate capacitance values (fF/um, fF/um²)
            - [ ] Validate resistance values (ohm/sq)
            - [ ] Confirm rcx.cal commands are properly formatted
            - [ ] Test with Calibre xCalibrate batch extraction
            """
            )
        else:
            st.info(
                "Configure parameters and click 'Generate xCalibrate Rules Decks' to create rule files"
            )

            with st.expander("Preview MIPT 2.0 Format"):
                example = """
// MIPT 2.0 Format Example
// Multi-Interval Parasitic Table for Capacitance

mipt_table capacitance {
    spacing = 0.5, 1.0, 2.0, 5.0;
    width = 0.5, 1.0, 2.0;
...
"""
    ...
    # Calibre xCalibrate Batch User Manual Section
    st.markdown("### 📖 Calibre xCalibrate Batch User Manual")
    st.markdown(
        "Reference Manual for Calibre xCalibrate Batch Parasitic Extraction Operations"
    )

    # xCalibrate PDF file path
    xcalibre_pdf_path = "/applications/Siemens/Calibre/docs_cal_2024.2_18.9/docs/pdfdocs/xcalibrat_batch_user.pdf"
    ...
    with col3:
        st.markdown("#### PDF Document Path")
        st.text_input(
            "Full path to Calibre xCalibrate Manual",
            value=xcalibre_pdf_path,
            disabled=True,
            help="Location of the Calibre xCalibrate Batch User Reference PDF document",
        )

    with col4:
        st.markdown("#### View Document")
        if st.button(
            "📄 View Calibre xCalibrate", type="primary", use_container_width=True
        ):
            if os.path.exists(xcalibre_pdf_path):
                if shutil.which("evince"):
                    try:
                        cmd = f'evince "{xcalibre_pdf_path}" &'
                        subprocess.Popen(
                            cmd,
                            shell=True,
                            stdout=subprocess.DEVNULL,
                        ...
                        st.success("Calibre xCalibrate Manual opened in evince!")
                    except Exception as e:
                        st.error(f"Error opening PDF: {str(e)}")
                else:
                    st.error("evince PDF viewer not found in PATH.")
                ...
                st.error(f"PDF file not found: {xcalibre_pdf_path}")
                st.info(
                    "Please ensure the xcalibrat_batch_user.pdf file is at the specified location."
                )

    st.markdown("---")

    # MIPT 2.0 Format Reference Section
    st.markdown("### 📋 MIPT 2.0 Format Reference")

    with st.expander("📐 MIPT 2.0 File Structure", expanded=True):
        st.markdown(
            """
        #### MIPT 2.0 Table Structure
        The Multi-Interval Parasitic Table (MIPT) 2.0 format defines capacitance and resistance values
        as functions of geometry parameters like width and spacing.
        
        **Key Components:**
        - **spacing**: Array of spacing values (um)
        - **width**: Array of width values (um)  
        - **table_data**: 2D matrix of parasitic values
        
        #### File Extensions:
        - `.C` - Capacitance tables (fF/um for fringe, fF/um² for area)
        - `.R` - Resistance tables (ohm/sq)
        - `.cal` - Calibre rule file with extraction commands
        """
        )

    with st.expander("⚡ Example MIPT 2.0 Capacitance Table (SCMOS_rules.C)"):
        mipt_example = """
// ============================================================
// SCMOS SUBM MIPT 2.0 Capacitance Rules
// Generated for 0.5um SCMOS Process
// ============================================================

// Metal1 to Substrate Area Capacitance
mipt_table M1_SUB_area {
    type = AREA;
    unit = fF/um^2;
    
    spacing = 0.0;  // Not applicable for area cap
...
// Metal1 Fringe Capacitance vs Spacing
mipt_table M1_fringe {
    type = FRINGE;
    unit = fF/um;
    
    spacing = 0.5, 1.0, 1.5, 2.0, 5.0;
    width = 0.5, 1.0, 2.0;
    
    table_data {
        0.045, 0.035, 0.030, 0.028, 0.025;
        0.050, 0.040, 0.035, 0.032, 0.028;
        0.055, 0.045, 0.040, 0.036, 0.032;
    }
}

// Metal1 to Metal2 Coupling Capacitance
mipt_table M1_M2_coupling {
    type = COUPLING;
    unit = fF/um^2;
    
    spacing = 0.0;  // Area cap, spacing independent
    width = 0.5, 1.0, 2.0, 5.0;
...
// Metal2 Fringe Capacitance
mipt_table M2_fringe {
    type = FRINGE;
    unit = fF/um;
    ...
    table_data {
        0.040, 0.032, 0.028, 0.025, 0.022;
        0.045, 0.036, 0.032, 0.028, 0.025;
        0.048, 0.040, 0.035, 0.032, 0.028;
    }
}
"""
        st.code(mipt_example, language="text")

    with st.expander("🔗 Example MIPT 2.0 Resistance Table (SCMOS_rules.R)"):
        res_example = """
// ============================================================
// SCMOS SUBM MIPT 2.0 Resistance Rules
// Generated for 0.5um SCMOS Process
// ============================================================

// Metal1 Sheet Resistance
mipt_table M1_resistance {
    type = SHEET_RES;
    unit = ohm/sq;
    
    spacing = 0.0;  // Not applicable
    width = 0.5, 1.0, 2.0, 5.0, 10.0, 20.0;
...
mipt_table M3_resistance {
    type = SHEET_RES;
    unit = ohm/sq;
    
    spacing = 0.0;
    width = 1.0, 2.0, 5.0, 10.0, 20.0;
...
// Poly Sheet Resistance (salicided)
mipt_table POLY_resistance {
    type = SHEET_RES;
    unit = ohm/sq;
...
// Via Resistance
mipt_table VIA_resistance {
    type = VIA_RES;
    unit = ohm;
    
    spacing = 0.0;
    width = 1.0;  // Single via size
...
"""
        st.code(res_example, language="text")

    with st.expander("⚙️ Example xCalibrate Command File (SCMOS_rcx.cal)"):
        rcx_example = """
// ============================================================
// SCMOS SUBM xCalibrate Batch Command File
// Generated for 0.5um SCMOS Process
// ============================================================

// Load MIPT tables
LOAD_CAP_TABLE SCMOS_rules.C
LOAD_RES_TABLE SCMOS_rules.R
...
// Output settings
PEX NETLIST FORMAT HSPICE
PEX NETLIST FILE "design_pex.sp"
PEX NETLIST GROUND NET 0

// Reduction settings
PEX REDUCE MINIMUM R 0.1
PEX REDUCE MINIMUM C 0.01f
PEX REDUCE MINIMUM CC 0.005f

// Hierarchical extraction
PEX HIERARCHY YES

// Coupling capacitance settings
PEX COUPLING CMAX 3.0
PEX COUPLING NONE "VDD"
PEX COUPLING NONE "VSS"
...
// Extraction options
PEX FIELDSolver FAST
PEX ITF yes

// Output options
PEX NETLIST UNNAMED NETS YES
PEX NETLIST NOISE ANALYSIS NO
"""
        st.code(rcx_example, language="text")


# ============== APP 5: CALIBRE PEX/xRC DECK GENERATOR ==============


def generate_xcalibre_decks(
    extraction_type, include_r, include_c, include_coupling, hierarchical, mipt_version
):
    """Generate xCalibrate batch rule deck files (SCMOS_rules.C, SCMOS_rules.R, SCMOS_rcx.cal)"""

    # Generate SCMOS_rules.C (Capacitance file)
    cap_lines = []
    cap_lines.append("// ============================================================")
    cap_lines.append("// SCMOS SUBM MIPT 2.0 Capacitance Rules")
    cap_lines.append(f"// Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    cap_lines.append("// Process: 0.5um SCMOS")
    cap_lines.append("// ============================================================")
    cap_lines.append("")

    if include_c:
        # Metal1 Area Capacitance
        cap_lines.append("// Metal1 to Substrate Area Capacitance")
        cap_lines.append("mipt_table M1_SUB_area {")
        cap_lines.append("    type = AREA;")
        cap_lines.append("    unit = fF/um^2;")
        cap_lines.append("    ")
        cap_lines.append("    spacing = 0.0;")
        cap_lines.append("    width = 0.5, 1.0, 2.0, 5.0, 10.0;")
        cap_lines.append("    ")
        cap_lines.append("    table_data {")
        cap_lines.append("        0.030, 0.030, 0.030, 0.030, 0.030;")
        cap_lines.append("    }")
        cap_lines.append("}")
        cap_lines.append("")

        # Metal1 Fringe Capacitance
        cap_lines.append("// Metal1 Fringe Capacitance vs Spacing")
        cap_lines.append("mipt_table M1_fringe {")
        cap_lines.append("    type = FRINGE;")
        cap_lines.append("    unit = fF/um;")
        cap_lines.append("    ")
        cap_lines.append("    spacing = 0.5, 1.0, 1.5, 2.0, 5.0;")
        cap_lines.append("    width = 0.5, 1.0, 2.0;")
        cap_lines.append("    ")
        cap_lines.append("    table_data {")
        cap_lines.append("        0.045, 0.035, 0.030, 0.028, 0.025;")
        cap_lines.append("        0.050, 0.040, 0.035, 0.032, 0.028;")
        cap_lines.append("        0.055, 0.045, 0.040, 0.036, 0.032;")
        cap_lines.append("    }")
        cap_lines.append("}")
        cap_lines.append("")

        # Metal1 to Metal2 Coupling
        cap_lines.append("// Metal1 to Metal2 Coupling Capacitance")
        cap_lines.append("mipt_table M1_M2_coupling {")
        cap_lines.append("    type = COUPLING;")
        cap_lines.append("    unit = fF/um^2;")
        cap_lines.append("    ")
        cap_lines.append("    spacing = 0.0;")
        cap_lines.append("    width = 0.5, 1.0, 2.0, 5.0;")
        cap_lines.append("    ")
        cap_lines.append("    table_data {")
        cap_lines.append("        0.050, 0.050, 0.050, 0.050;")
        cap_lines.append("    }")
        cap_lines.append("}")
        cap_lines.append("")

        # Metal2 Fringe
        cap_lines.append("// Metal2 Fringe Capacitance")
        cap_lines.append("mipt_table M2_fringe {")
        cap_lines.append("    type = FRINGE;")
        cap_lines.append("    unit = fF/um;")
        cap_lines.append("    ")
        cap_lines.append("    spacing = 0.5, 1.0, 1.5, 2.0, 5.0;")
        ...
        cap_lines.append("    ")
        cap_lines.append("    table_data {")
        cap_lines.append("        0.040, 0.032, 0.028, 0.025, 0.022;")
        cap_lines.append("        0.045, 0.036, 0.032, 0.028, 0.025;")
        cap_lines.append("        0.048, 0.040, 0.035, 0.032, 0.028;")
        cap_lines.append("    }")
        cap_lines.append("}")
        cap_lines.append("")

        # Metal3 Fringe
        cap_lines.append("// Metal3 Fringe Capacitance")
        cap_lines.append("mipt_table M3_fringe {")
        cap_lines.append("    type = FRINGE;")
        cap_lines.append("    unit = fF/um;")
        cap_lines.append("    ")
        cap_lines.append("    spacing = 0.5, 1.0, 1.5, 2.0, 5.0;")
        cap_lines.append("    width = 1.0, 2.0, 5.0;")
        cap_lines.append("    ")
        cap_lines.append("    table_data {")
        cap_lines.append("        0.035, 0.028, 0.024, 0.022, 0.020;")
        cap_lines.append("        0.040, 0.032, 0.028, 0.025, 0.022;")
        cap_lines.append("        0.042, 0.035, 0.030, 0.028, 0.025;")
        cap_lines.append("    }")
        cap_lines.append("}")
        cap_lines.append("")
    ...
    # Generate SCMOS_rules.R (Resistance file)
    res_lines = []
    res_lines.append("// ============================================================")
    res_lines.append("// SCMOS SUBM MIPT 2.0 Resistance Rules")
    res_lines.append(f"// Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    res_lines.append("// Process: 0.5um SCMOS")
    res_lines.append("// ============================================================")
    res_lines.append("")

    if include_r:
        # Metal1 Resistance
        res_lines.append("// Metal1 Sheet Resistance")
        res_lines.append("mipt_table M1_resistance {")
        res_lines.append("    type = SHEET_RES;")
        res_lines.append("    unit = ohm/sq;")
        res_lines.append("    ")
        res_lines.append("    spacing = 0.0;")
        res_lines.append("    width = 0.5, 1.0, 2.0, 5.0, 10.0, 20.0;")
        res_lines.append("    ")
        res_lines.append("    table_data {")
        res_lines.append("        0.080, 0.080, 0.080, 0.080, 0.080, 0.080;")
        res_lines.append("    }")
        res_lines.append("}")
        res_lines.append("")

        # Metal2 Resistance
        res_lines.append("// Metal2 Sheet Resistance")
        res_lines.append("mipt_table M2_resistance {")
        res_lines.append("    type = SHEET_RES;")
        res_lines.append("    unit = ohm/sq;")
        res_lines.append("    ")
        res_lines.append("    spacing = 0.0;")
        ...
        res_lines.append("    ")
        res_lines.append("    table_data {")
        res_lines.append("        0.060, 0.060, 0.060, 0.060, 0.060, 0.060;")
        res_lines.append("    }")
        res_lines.append("}")
        res_lines.append("")

        # Metal3 Resistance
        res_lines.append("// Metal3 Sheet Resistance")
        res_lines.append("mipt_table M3_resistance {")
        res_lines.append("    type = SHEET_RES;")
        res_lines.append("    unit = ohm/sq;")
        res_lines.append("    ")
        res_lines.append("    spacing = 0.0;")
        res_lines.append("    width = 1.0, 2.0, 5.0, 10.0, 20.0;")
        res_lines.append("    ")
        res_lines.append("    table_data {")
        res_lines.append("        0.050, 0.050, 0.050, 0.050, 0.050;")
        res_lines.append("    }")
        res_lines.append("}")
        res_lines.append("")

        # Poly Resistance
        res_lines.append("// Poly Sheet Resistance (salicided)")
        res_lines.append("mipt_table POLY_resistance {")
        res_lines.append("    type = SHEET_RES;")
        res_lines.append("    unit = ohm/sq;")
        res_lines.append("    ")
        res_lines.append("    spacing = 0.0;")
        res_lines.append("    width = 0.5, 1.0, 2.0, 5.0, 10.0;")
        res_lines.append("    ")
        res_lines.append("    table_data {")
        res_lines.append("        8.0, 8.0, 8.0, 8.0, 8.0;")
        res_lines.append("    }")
        res_lines.append("}")
        res_lines.append("")

        # Via Resistance
        res_lines.append("// Via Resistance")
        res_lines.append("mipt_table VIA_resistance {")
        res_lines.append("    type = VIA_RES;")
        res_lines.append("    unit = ohm;")
        res_lines.append("    ")
        res_lines.append("    spacing = 0.0;")
        res_lines.append("    width = 1.0;")
        res_lines.append("    ")
        res_lines.append("    table_data {")
        res_lines.append("        0.5;")
        res_lines.append("    }")
        res_lines.append("}")
        res_lines.append("")

        # Contact Resistance
        res_lines.append("// Contact Resistance")
        res_lines.append("mipt_table CONTACT_resistance {")
        res_lines.append("    type = CONTACT_RES;")
        res_lines.append("    unit = ohm;")
        res_lines.append("    ")
        res_lines.append("    spacing = 0.0;")
        res_lines.append("    width = 1.0;")
    ...
    # Generate SCMOS_rcx.cal (Command file)
    rcx_lines = []
    rcx_lines.append("// ============================================================")
    rcx_lines.append("// SCMOS SUBM xCalibrate Batch Command File")
    rcx_lines.append(f"// Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    rcx_lines.append("// Process: 0.5um SCMOS")
    rcx_lines.append("// ============================================================")
    rcx_lines.append("")
    rcx_lines.append("// Load MIPT tables")
    rcx_lines.append("LOAD_CAP_TABLE SCMOS_rules.C")
    rcx_lines.append("LOAD_RES_TABLE SCMOS_rules.R")
    rcx_lines.append("")
    rcx_lines.append("// Layout settings")
    rcx_lines.append('LAYOUT PRIMARY "TOP"')
    rcx_lines.append('LAYOUT PATH "design.gds"')
    rcx_lines.append("LAYOUT SYSTEM GDSII")
    rcx_lines.append("")
    ...
    rcx_lines.append(f"// PEX mode: {extraction_type}")
    rcx_lines.append(f"PEX EXTRACT {mode_map.get(extraction_type, 'RC')}")
    rcx_lines.append("")

    rcx_lines.append("// Output settings")
    rcx_lines.append("PEX NETLIST FORMAT HSPICE")
    rcx_lines.append('PEX NETLIST FILE "design_pex.sp"')
    rcx_lines.append("PEX NETLIST GROUND NET 0")
    rcx_lines.append("")
    rcx_lines.append("// Reduction settings")
    rcx_lines.append("PEX REDUCE MINIMUM R 0.1")
    rcx_lines.append("PEX REDUCE MINIMUM C 0.01f")
    if include_coupling:
        rcx_lines.append("PEX REDUCE MINIMUM CC 0.005f")
    rcx_lines.append("")
    rcx_lines.append("// Hierarchical extraction")
    if hierarchical:
        rcx_lines.append("PEX HIERARCHY YES")
    else:
        rcx_lines.append("PEX HIERARCHY NO")
    rcx_lines.append("")

    if include_coupling:
        rcx_lines.append("// Coupling capacitance settings")
        rcx_lines.append("PEX COUPLING CMAX 3.0")
        rcx_lines.append('PEX COUPLING NONE "VDD"')
        rcx_lines.append('PEX COUPLING NONE "VSS"')
        rcx_lines.append("")

    rcx_lines.append("// Layer mapping")
    rcx_lines.append("LAYER M1 49")
    rcx_lines.append("LAYER M2 51")
    rcx_lines.append("LAYER M3 62")
    rcx_lines.append("LAYER VIA1 50")
    rcx_lines.append("LAYER VIA2 61")
    rcx_lines.append("")
    rcx_lines.append("// Extraction options")
    rcx_lines.append("PEX FIELDSolver FAST")
    rcx_lines.append("PEX ITF yes")
    rcx_lines.append("")
    rcx_lines.append("// Output options")
    rcx_lines.append("PEX NETLIST UNNAMED NETS YES")
    rcx_lines.append("PEX NETLIST NOISE ANALYSIS NO")
    rcx_lines.append("")
    rcx_lines.append("// ============================================================")
    rcx_lines.append("// END OF xCalibrate COMMAND FILE")
    rcx_lines.append("// ============================================================")
    rcx_lines.append("")

    rcx_content = "\n".join(rcx_lines)

    return cap_content, res_content, rcx_content


def calibre_pex_generator_app():
    """Calibre PEX/xRC Rules Deck Generator - Generate parasitic extraction rule decks for SCMOS SUBM"""
    st.markdown("### 🔬 Siemens Calibre PEX/xRC Rules Deck Generator")
    st.markdown(
    ...
    with col2:
        st.subheader("Generated PEX Rules Deck")
        ...
        if st.session_state.pex_deck:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            st.download_button(
                label="📥 Download Calibre PEX Deck (.svrf)",
...
"""
    ...
    st.markdown("### 📖 Calibre xRC User Manual")
    ...
    with col4:
        st.markdown("#### View Document")
        if st.button("📄 View Calibre xRC", type="primary", use_container_width=True):
    ...
    # PEX Basics Summary
    st.markdown("### 📋 PEX Basic Operations Summary")

    with st.expander("📐 Resistance Extraction", expanded=True):
        st.markdown(
            """
        ...
        - **Example**: `RESISTANCE METAL1 0.08` - 0.08 Ω/sq for metal1
        
        #### Via Resistance
        - **Syntax**: `RESISTANCE via_layer <resistance_value>`
        - **Units**: Ohms per via (Ω)
        - **Example**: `RESISTANCE VIA 0.5` - 0.5 Ω per via
        
        #### Key Parameters:
        - Sheet resistance varies by layer (poly ~ 5-10 Ω/□, metal ~ 0.03-0.1 Ω/□)
        - Via resistance typically 0.1-1.0 Ω per cut
        - Temperature coefficients can be specified
        """
        )

    with st.expander("⚡ Capacitance Extraction"):
        st.markdown(
            """
        #### CAPACITANCE Statements
        - **Purpose**: Extract parasitic capacitances between layers
        - **Area Capacitance**: `CAPACITANCE layer1 layer2 <value>` (F/μm²)
        - **Fringe Capacitance**: `FRINGE layer1 layer2 <value>` (F/μm)
        ...
        #### Typical Values (0.5μm process):
        - Metal1 to Substrate: ~0.03 fF/μm²
        - Metal1 to Metal2: ~0.05 fF/μm²  
        - Poly to Substrate: ~0.1 fF/μm²
        - Fringe (edge): ~0.05 fF/μm
        """
        )

    with st.expander("🔗 Extraction Flow"):
        st.markdown(
            """
        #### PEX Process Flow
    ...
    with st.expander("⚙️ PEX Control Statements"):
        st.markdown(
            """
        #### PEX Options
...
def generate_pex_deck(
    ...
    lines = []
    lines.append("////////////////////////////////////////////////////////")
    lines.append("// MOSIS SCMOS SUBM Calibre PEX Rules Deck")
    lines.append(f"// Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if include_r:
        ...
        lines.append("// --------------------------------------------")
        lines.append("// Sheet resistance values (Ω/sq) for 0.5um SCMOS")
        lines.append("")
        lines.append("// Poly layer: ~5-10 Ω/sq (salicided)")
        lines.append("RESISTANCE POLY 8.0")
        lines.append("")
        lines.append("// Metal layers: ~0.03-0.08 Ω/sq")
        lines.append("RESISTANCE METAL1 0.08")
        lines.append("RESISTANCE METAL2 0.06")
        lines.append("RESISTANCE METAL3 0.05")
        lines.append("")
        lines.append("// Via resistance: ~0.1-1.0 Ω per cut")
    ...
    if include_c:
        ...
        lines.append("// --------------------------------------------")
        lines.append("// Values in fF/μm² for 0.5um SCMOS process")
        lines.append("")
...
def calibre_opc_generator_app():
    """Calibre OPC Rules Deck Generator - Generate optical proximity correction rule decks for SCMOS SUBM"""
    st.markdown("### 🎯 Siemens Calibre OPC Rules Deck Generator")
    st.markdown(
    ...
    with col1:
        ...
        st.markdown("---")
        if st.button("Generate OPC Rules Deck", type="primary"):
    ...
    with col2:
        st.subheader("Generated OPC Rules Deck")
        ...
        if st.session_state.opc_deck:
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            st.download_button(
                label="📥 Download Calibre OPC Rules Deck (.svrf)",
            ...
            st.info(
                "Configure parameters and click 'Generate OPC Rules Deck' to create rule deck"
            )

            with st.expander("Preview OPC Format"):
                example = """
// OPC Rules Deck Example
LAYER POLY 46
...
"""
    ...
    st.markdown("### 📖 Calibre nmOPC User Reference")
    st.markdown("Reference Manual for Calibre Optical Proximity Correction Operations")

    # nmOPC PDF file path
    nmopc_pdf_path = "/applications/Siemens/Calibre/docs_cal_2024.2_18.9/docs/pdfdocs/calbr_nmopc_useref.pdf"
    ...
    with col4:
        st.markdown("#### View Document")
        if st.button(
            "📄 View Calibre nmOPC User Reference",
            type="primary",
            use_container_width=True,
        ):
    ...
    st.markdown("### 📖 Calibre OPCpro User Reference")
    st.markdown("Advanced OPC Professional User Reference Guide")

    # OPCpro PDF file path
    opcpro_pdf_path = "/applications/Siemens/Calibre/docs_cal_2024.2_18.9/docs/pdfdocs/calbr_opcpro_useref.pdf"
    ...
    with col6:
        st.markdown("#### View Document")
        if st.button(
            "📄 View Calibre OPCpro User Reference",
            type="primary",
            use_container_width=True,
        ):
    ...
    st.markdown("### 📖 Calibre OPCverify User Reference")
    st.markdown("OPC Verification and Printability Check User Reference Guide")

    # OPCverify PDF file path
    opcv_pdf_path = "/applications/Siemens/Calibre/docs_cal_2024.2_18.9/docs/pdfdocs/calbr_opcv_useref.pdf"
    ...
    with col8:
        st.markdown("#### View Document")
        if st.button(
            "📄 View Calibre OPCverify User Reference",
            type="primary",
            use_container_width=True,
        ):
    ...
    # OPC Basics Summary
    st.markdown("### 📋 OPC Basic Operations Summary")

    with st.expander("📐 OPC Fundamentals", expanded=True):
        st.markdown(
            """
        #### What is OPC?
    ...
    with st.expander("⚡ Fragmentation and Correction"):
        st.markdown(
            """
    ...
    with st.expander("🔗 Model-Based OPC"):
        st.markdown(
            """
...
def generate_opc_deck(
    ...
    lines = []
    lines.append("////////////////////////////////////////////////////////")
    lines.append("// MOSIS SCMOS SUBM Calibre OPC Rules Deck")
    lines.append(f"// Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def dlm_reference_app():
    ...
    st.markdown("### 📖 Design Layout Manual Reference")
    ...
    with col2:
        st.markdown("#### View Document")
        if st.button("📄 View DLM", type="primary", use_container_width=True):
    ...
    with col4:
        ...
        if st.button(
            "🔗 Open URL in Browser", type="secondary", use_container_width=True
        ):
            import webbrowser
    ...
    with st.expander("🔧 Technology Overview", expanded=True):
        ...
        **Process Node**: Scalable from 0.5μm to sub-micron
        **Design Units**: All dimensions in microns, scaled by lambda
        **Grid Resolution**: All geometry must be on 0.5 lambda grid
        """)

    with st.expander("📐 Critical Design Rules Summary"):
        st.markdown("""
        #### N-WELL Rules
        - **Minimum Width**: 12λ (6.0 μm @ λ=0.5μm)
        - **Same Potential Spacing**: 6λ (3.0 μm)
        - **Different Potential Spacing**: 18λ (9.0 μm)
        
        #### ACTIVE Area Rules
        - **Minimum Width**: 3λ (1.5 μm)
        - **Minimum Spacing**: 3λ (1.5 μm)
        - **To Well Edge**: 6λ (3.0 μm)
        
        #### POLY Rules
        - **Minimum Width**: 2λ (1.0 μm)
        - **Field Spacing**: 3λ (1.5 μm)
        - **Extension over Active**: 2λ (1.0 μm)
        
        #### Contact Rules
        - **Size**: 2λ × 2λ (1.0 × 1.0 μm)
        - **Active Overlap**: 1.5λ (0.75 μm)
        - **Spacing**: 3λ (1.5 μm)
        ...
        #### METAL3 Rules
        - **Minimum Width**: 6λ (3.0 μm)
        - **Minimum Spacing**: 3λ (1.5 μm)
        """)

    with st.expander("⚠️ Important Design Notes"):
        st.markdown("""
        ...
        3. **Wide metal spacing rules** apply for metal width > 10λ
        ...
        #### Best Practices
        - Always use minimum grid (0.5λ) for all geometry
        - Place substrate/well contacts regularly
        - Avoid acute angles in layout
        - Use consistent lambda value (default: 0.5μm)
        """)

    st.markdown("---")

    # Layer Reference Summary
    st.markdown("### 📊 Layer Stack Summary")
...
def generate_inverter_layout_images():
    ...
    ax_cross.text(
        10,
        12,
        "Operation: Gate LOW (0V) → Channel conducts → VDD flows to Output",
        ha="center",
        va="center",
        fontsize=10,
        style="italic",
...
def inverter_layout_intro_app():
    ...
    st.markdown("### ⚡ Introduction to Layout of an Inverter")
    st.markdown(
    ...
    with col1:
        st.markdown(
            ...
            <div style="background-color: #e3f2fd; padding: 15px; border-radius: 8px; border-left: 4px solid #2196F3;">
                <h4 style="margin: 0; color: #1976d2;">🔧 Basic Structure</h4>
    ...
    with col2:
        st.markdown(
            ...
            <div style="background-color: #f3e5f5; padding: 15px; border-radius: 8px; border-left: 4px solid #9c27b0;">
                <h4 style="margin: 0; color: #7b1fa2;">📐 Layout Efficiency</h4>
                <p style="margin: 8px 0 0 0; font-size: 14px;">
    ...
    with col3:
        st.markdown(
            ...
            <div style="background-color: #e8f5e9; padding: 15px; border-radius: 8px; border-left: 4px solid #4caf50;">
                <h4 style="margin: 0; color: #388e3c;">✅ DRC Clean</h4>
                <p style="margin: 8px 0 0 0; font-size: 14px;">
    ...
    with st.expander("📊 Inverter Schematic & Concept", expanded=True):
        st.markdown("""
        ...
        **Key Characteristics:**
        ...
        |-------------|------------|------------|---------------|
        | LOW (0)     | ON         | OFF        | HIGH (1) → VDD|
        | HIGH (1)    | OFF        | ON         | LOW (0) → VSS |
        
        **Logic Function**: Vout = NOT(Vin) or Vout = Vin̄
        """)

    st.markdown("---")

    # Layout Structure Section
    with st.expander("🏗️ Layout Structure & Floorplan", expanded=True):
        st.markdown("""
        ...
        │                                                    │
        │  ┌────────────────────────────────────────────┐    │
        │  │           N_WELL (P-substrate tap)         │    │ ← N-Well
        │  │  ┌──────────────────────────────────────┐  │    │
        │  │  │  ACTIVE      PMOS    ACTIVE          │  │    │ ← P+ Diffusion
        │  │  │  (Source)    │Gate│   (Drain)        │  │    │
        │  │  │     ●────────┼────┼────────●         │  │    │
        │  │  └──────────────┼────┼──────────────────┘  │    │
        │  │                 │POLY│                     │    │ ← Shared Gate
        │  │  ┌──────────────┼────┼──────────────────┐  │    │
        │  │  │  ACTIVE      │    │   ACTIVE         │  │    │ ← N+ Diffusion
        │  │  │  (Drain)     │    │   (Source)       │  │    │
        │  │  │     ●────────┼────┼────────●         │  │    │
        │  │  │              NMOS │                  │  │    │
        │  │  └──────────────────────────────────────┘  │    │
        ...
        │   (Power)   │   (Power)    │ (Output)│(Output)│ (Ground)│
        ├─────────────┼──────────────┼─────────┼────────┼─────────┤
        │   VIA       │    VIA       │   VIA   │  VIA   │   VIA   │
        ├─────────────┼──────────────┼─────────┼────────┼─────────┤
        ...
        ├─────────────┼──────────────┼─────────┼────────┼─────────┤
        │   N_WELL    │   P+ ACTIVE  │   POLY  │N+ACTIV │   P_WELL│
        │   (Well)    │  (PMOS S/D)  │ (Gate)  │(NMOS)  │  (Sub)  │
        └─────────────┴──────────────┴─────────┴────────┴─────────┘
        ```
        """)
        ...
        3. **Source/Drain Arrangement**:
           - PMOS Source → VDD (top)
           - PMOS Drain → Output (middle)
           - NMOS Drain → Output (middle)
           - NMOS Source → VSS (bottom)
    ...
    with st.expander("🎨 Layer-by-Layer Layout Guide"):
        st.markdown("""
        ...
        **Step 1: Substrate & Well Definition**
        ...
        - Ensure sufficient well-to-well spacing (18λ for different potential)
        ...
        **Step 2: Active Area (Diffusion)**
        ...
        - Minimum active width: 3λ, spacing: 3λ
        ...
        **Step 3: Gate Poly Definition**
        ...
        Layer: POLY (GDS 46)
              ...
              ┌──────────────────┐
              │                  │
              │    POLY GATE     │  ← Width: 2λ (1.0μm @ λ=0.5μm)
              │    ━━━━━━━━━━    │    Length: L (e.g., 0.5μm)
              │                  │    Extends 2λ beyond active
              └──────────────────┘
        ```
        - Draw poly gate perpendicular to diffusion
        - Poly overlaps active by 2λ minimum
        - Active overlaps poly by 3λ minimum
        ...
        **Step 4: Contacts**
        ...
        Requirements:
        - Contact size: 2λ × 2λ (exact)
        - Active overlap: 1.5λ minimum
        - Poly overlap: 1.5λ minimum  
        - Contact spacing: 3λ minimum
        - Poly-to-contact spacing: 2λ minimum
        ```
        ...
        **Step 5: Metal1 Interconnect**
        ...
        Design Rules:
        - Metal1 width: 3λ minimum
        - Metal1 spacing: 3λ minimum
        - Metal1 overlap contact: 1λ minimum
        ```
    ...
    with st.expander("⚠️ Critical Design Considerations"):
        st.markdown("""
        ...
        **3. Well Contact Placement:**
        ...
        - Well contact to active spacing: 6λ
    ...
    with st.expander("📏 Layout Metrics & Sizing"):
        st.markdown("""
        #### Typical Inverter Dimensions (λ = 0.5μm)
        
        **Minimum Size Inverter (Wn = 3μm, Wp = 6μm):**
        
        | Parameter | Value (λ) | Value (μm) | Notes |
        |-----------|-----------|------------|-------|
        | Total Width | ~30λ | ~15μm | Depends on Wp/Wn |
        | Total Height | ~25λ | ~12.5μm | VDD to VSS rails |
        | Gate Length (L) | 2λ | 1.0μm | Both transistors |
        | NMOS Width (Wn) | 6λ | 3.0μm | Minimum typical |
        | PMOS Width (Wp) | 12λ | 6.0μm | 2× Wn for matching |
        | Cell Area | ~750λ² | ~187μm² | Rough estimate |
        ...
        **Scaling Considerations:**
        ```
        For β = Wp/Wn = 2 (balanced rise/fall times):
        
        If Wn = 3μm (6λ), then Wp = 6μm (12λ)
        
        Area impact:
        - PMOS occupies ~2× the width of NMOS
        - Total width ≈ 3× minimum NMOS width
    ...
    with st.expander("🔬 Advanced Layout Techniques"):
        st.markdown("""
        ...
        ┌────────────────────────────────────┐
        │         Very Wide ACTIVE           │  ← Large Cjs
        │    ━━━━━━━━━━━━━━━━━━━━━━━━━━━━    │
        └────────────────────────────────────┘
        
        Multi-Finger Layout (Efficient):
        ┌───────┬───────┬───────┬───────┐
        │ ACTIVE│  POLY │ ACTIVE│  POLY │
        │  S/D  │ Gate1 │  D/S  │ Gate2 │  ← Shared S/D reduces cap
        ├───────┼───────┼───────┼───────┤
        │  S/D  │  POLY │  D/S  │  POLY │
        │       │ Gate3 │       │ Gate4 │
        └───────┴───────┴───────┴───────┘
        ...
        Best Practices:
        - Wide metal for power (6λ minimum for Metal1)
        - Multiple via stacks (2×2 or 3×3)
    ...
    # DRC Checklist
    with st.expander("✅ DRC Verification Checklist"):
        st.markdown("""
        #### Pre-Layout Completion Checklist
        
        **N-Well Checks:**
        - [ ] N-Well covers entire PMOS area
        - [ ] N-Well to N-Well spacing (18λ for different potential)
        - [ ] N-Well overlap of P+ active (6λ minimum)
        - [ ] Well contacts placed and sized correctly
        
        **Active Area Checks:**
        - [ ] Active width ≥ 3λ
        - [ ] Active spacing ≥ 3λ
        - [ ] Active to well edge ≥ 6λ (S/D) or 3λ (substrate contact)
        - [ ] N+ and P+ select layers correct
        
        **Poly Gate Checks:**
        - [ ] Poly width ≥ 2λ
        - [ ] Poly spacing ≥ 3λ
        - [ ] Poly overlap of active ≥ 2λ
        - [ ] Active overlap of poly ≥ 3λ
        - [ ] Field poly to active ≥ 1λ
        
        **Contact Checks:**
        - [ ] Contact size exactly 2λ × 2λ
        - [ ] Contact spacing ≥ 3λ
        - [ ] Active overlap of contact ≥ 1.5λ
        - [ ] Poly overlap of contact ≥ 1.5λ
        - [ ] Contact to gate spacing ≥ 2λ
        
        **Metal Checks:**
        - [ ] Metal1 width ≥ 3λ
        - [ ] Metal1 spacing ≥ 3λ
        - [ ] Metal1 overlap of contact ≥ 1λ
        - [ ] Metal1 wide spacing (6λ for width > 10λ)
    ...
    st.markdown(
        ...
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 10px; margin-top: 20px;">
            <h3 style="color: white; margin: 0;">🎯 Key Takeaways</h3>
    ...
    with st.expander("🔍 Mask-Level Layout Visualization", expanded=True):
...
def main():
    st.markdown(
        '<div class="main-header">🔧 MOSIS_SCMOS PDK PV Automation QA & Validation Dashboard</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    app_mode = st.sidebar.radio(
        "Select Application",
        [
            "QA Error Cell GDS Generator",
            "Calibre DRC Rules Deck Generator",
            "Calibre LVS Rules Deck Generator",
            "Calibre PEX/xCalibrate Rules Deck Generator",
            "Calibre PEX/xRC Rules Deck Generator",
            "Calibre OPC Rules Deck Generator",
            "Layer Map Reference",
    ...
    # Get OS info in clean format (e.g., "Rocky Linux 9.6")
    try:
        os_release = platform.freedesktop_os_release()
        os_info = (
            f"{os_release.get('NAME', 'Unknown')} {os_release.get('VERSION_ID', '')}"
        )
    except:
        os_info = platform.system() + " " + platform.release()

    # Get local IP address using hostname -I
    try:
    ...
    try:
        with open("/proc/meminfo", "r") as f:
            lines = f.readlines()
            total_mem = int(lines[0].split()[1]) / (1024**2)  # Convert to GB
            available_mem = int(lines[2].split()[1]) / (1024**2)  # Convert to GB
            mem_info = f"{available_mem:.1f}GB / {total_mem:.1f}GB"
    except:
        mem_info = "N/A"
    ...
    # Display in compact styled containers
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.9rem;">
            <strong>👤 User:</strong> <span style="color: #1f77b4; font-family: monospace;">{username}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.9rem;">
            <strong>🌐 Host:</strong> <span style="color: #1f77b4; font-family: monospace;">{hostname}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.85rem;">
            <strong>🖥️ OS:</strong> <span style="color: #1f77b4; font-family: monospace; font-size: 0.8rem;">{os_info}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.9rem;">
            <strong>📍 Local IP:</strong> <span style="color: #1f77b4; font-family: monospace;">{local_ip}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.9rem;">
            <strong>🌍 Public IP:</strong> <span style="color: #1f77b4; font-family: monospace;">{public_ip}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.9rem;">
            <strong>💾 Memory:</strong> <span style="color: #1f77b4; font-family: monospace;">{mem_info}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; margin-bottom: 5px; font-size: 0.9rem;">
            <strong>💿 Disk:</strong> <span style="color: #1f77b4; font-family: monospace;">{disk_info}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.sidebar.markdown(
        f"""
        <div style="background-color: #f0f2f6; padding: 6px 10px; border-radius: 5px; font-size: 0.9rem;">
            <strong>📅 Date:</strong> <span style="color: #1f77b4; font-family: monospace;">{current_date}</span>
        </div>
        """,
        unsafe_allow_html=True,
    )

    if app_mode == "QA Error Cell GDS Generator":
        qa_generator_app()
    elif app_mode == "Calibre DRC Rules Deck Generator":
        calibre_generator_app()
    elif app_mode == "Calibre LVS Rules Deck Generator":
        calibre_lvs_generator_app()
    elif app_mode == "Calibre PEX/xCalibrate Rules Deck Generator":
        calibre_xcalibre_generator_app()
    elif app_mode == "Calibre PEX/xRC Rules Deck Generator":
        calibre_pex_generator_app()
    elif app_mode == "Calibre OPC Rules Deck Generator":
